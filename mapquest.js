...
const {point} = require('./city.js'); // for a larger part of citylots.json

let streets = [];

const fetchLocationName = async () => {
    for (let i in point.features) {
        for (let j in point.features[i].geometry.coordinates[0]) {
            let lng = point.features[i].geometry.coordinates[0][j][0];
            let lat = point.features[i].geometry.coordinates[0][j][1];
            let fetchData;
            await fetch(
            ...
            .then((responseJson) => {
                ...
                fetchData.shift();
                if (!streets.includes(fetchData.join(' '))) {
                    streets.push(fetchData.join(' '));
                }
            }).catch((error) => console.log('Fetch problem: ' + error.message));
        };
    };
    console.log('ALL STREETS: ' + streets.join(', ') + '.');
    console.log(streets);
};
